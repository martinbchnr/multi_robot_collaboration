...
int main(int argc, char ** argv)
{
  ...
  nh_priv.getParam("robot_specifier", robot_specifier);
  nh_priv.param<double>("publish_frequency",publish_frequency,10);
  nh_priv.param<bool>("is_stamped", is_stamped, true); //rather publish stamped messages
  base_frame = robot_specifier+base_frame;
  ...
  while (nh.ok())
  {
    ...
    try
    {
      //ROS_INFO("This is the robot pose Publisher Before listener map_frame : [%s], base_frame : [%s]",map_frame.c_str(), base_frame.c_str());
      ...
      //ROS_INFO("This is the robot pose Publisher Before Catch map_frame : [%s], base_frame : [%s]",map_frame.c_str(), base_frame.c_str());
    }
    catch (tf::TransformException &ex)
    {
        ROS_DEBUG("[%s]: Encountered an exception while looking for transform.", ros::this_node::getName().c_str());
      // just continue on
    }
